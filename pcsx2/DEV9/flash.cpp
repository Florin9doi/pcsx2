...
u32 FLASHread32(u32 addr, int size)
{
	...
	switch (addr)
	{
		case FLASH_R_DATA:
			...
			counter += size;
			XFROM_LOG("DEV9: *FLASH DATA %dbit read 0x%08lX %s", size * 8, value, (ctrl & FLASH_PP_READ) ? "READ_ENABLE" : "READ_DISABLE");
			if (cmd == SM_CMD_READ3)
			{
		...
		case FLASH_R_CMD:
			XFROM_LOG("DEV9: *FLASH CMD %dbit read %s DENIED", size * 8, getCmdName(cmd));
			return cmd;

		case FLASH_R_ADDR:
			XFROM_LOG("DEV9: *FLASH ADDR %dbit read DENIED", size * 8);
			return 0;

		case FLASH_R_CTRL:
			XFROM_LOG("DEV9: *FLASH CTRL %dbit read 0x%08lX", size * 8, ctrl);
			return ctrl;

		case FLASH_R_ID:
			if (cmd == SM_CMD_READID)
			{
				XFROM_LOG("DEV9: *FLASH ID %dbit read 0x%08lX", size * 8, id);
			...
			else if (cmd == SM_CMD_GETSTATUS)
			{
				...
				XFROM_LOG("DEV9: *FLASH STATUS %dbit read 0x%08lX", size * 8, value);
				return value;
			} //else fall off
			return 0;

		default:
			XFROM_LOG("DEV9: *FLASH Unknown %dbit read at address %lx", size * 8, addr);
			return 0;
	}
}
...
void FLASHwrite32(u32 addr, u32 value, int size)
{
	...
	switch (addr & 0x1FFFFFFF)
	{
		case FLASH_R_DATA:
			...
			XFROM_LOG("DEV9: *FLASH DATA %dbit write 0x%08lX %s", size * 8, value, (ctrl & FLASH_PP_WRITE) ? "WRITE_ENABLE" : "WRITE_DISABLE");
		...
		case FLASH_R_CMD:
			if (!(ctrl & FLASH_PP_READY))
			{
				...
				{
					XFROM_LOG("DEV9: *FLASH CMD %dbit write %s ILLEGAL in busy mode - IGNORED", size * 8, getCmdName(value));
					break;
				}
			}
			if (cmd == SM_CMD_WRITEDATA)
			{
				...
				{
					XFROM_LOG("DEV9: *FLASH CMD %dbit write %s ILLEGAL after WRITEDATA cmd - IGNORED", size * 8, getCmdName(value));
					ctrl &= ~FLASH_PP_READY; //go busy, reset is needed
					break;
				}
			}
			XFROM_LOG("DEV9: *FLASH CMD %dbit write %s", size * 8, getCmdName(value));
			switch (value)
		...
		case FLASH_R_ADDR:
			XFROM_LOG("DEV9: *FLASH ADDR %dbit write 0x%08lX", size * 8, value);
			address |= (value & 0xFF) << (addrbyte == 0 ? 0 : (1 + 8 * addrbyte));
			addrbyte++;
			XFROM_LOG("DEV9: *FLASH ADDR = 0x%08lX (addrbyte=%d)", address, addrbyte);
			if (!(value & 0x100))
			{ // address is complete
				...
				addrbyte = 0; // address reset
				{
					...
					XFROM_LOG("DEV9: *FLASH ADDR = 0x%08lX (%d:%d:%d) (addrbyte=%d) FINAL", address, blocks, pages, bytes, addrbyte);
				}
			}
			break;

		case FLASH_R_CTRL:
			XFROM_LOG("DEV9: *FLASH CTRL %dbit write 0x%08lX", size * 8, value);
			ctrl = (ctrl & FLASH_PP_READY) | (value & ~FLASH_PP_READY);
			break;

		case FLASH_R_ID:
			XFROM_LOG("DEV9: *FLASH ID %dbit write 0x%08lX DENIED :P", size * 8, value);
			break;

		default:
			XFROM_LOG("DEV9: *FLASH Unkwnown %dbit write at address 0x%08lX= 0x%08lX IGNORED", size * 8, addr, value);
			break;
	}
}
